...
Future<GestationModel?> getGestationById(
String id) async {
final doc =
await _gestations.doc(id).get();

if (!doc.exists || doc.data() == null) {
return null;
}

return GestationModel.fromMap(
doc.data()!,
doc.id,
);
}

Future<GestationModel?>
getGestationActiveByBrebis(
String brebisId,
) async {
final snapshot = await _gestations
.where(
...
Future<void> addGestation(
GestationModel gestation) async {
await _gestations
.doc(gestation.id)
.set(gestation.toMap());
}

Future<void> updateGestation(
GestationModel gestation) async {
await _gestations
.doc(gestation.id)
.update(gestation.toMap());
}

Future<void> deleteGestation(
String id) async {
await _gestations.doc(id).delete();
}
Future<List<GestationModel>>
getGestationsByMouton(
String brebisId,
) async {
final snapshot = await _gestations
.where(
...
Future<int>
getNombreGestationsEnCours() async {
final snapshot = await _gestations
.where(
'statut',
isEqualTo: 'En cours',
...
Future<int>
getNombreGestationsTerminees() async {
final snapshot = await _gestations
.where(
'statut',
isEqualTo: 'Terminée',
...
Future<void> reactiverGestation(
String id) async {
await _gestations.doc(id).update({
'statut': 'En cours',
'active': true,
...
Future<void> terminerGestation({
  ...
  await _gestations.doc(id).update({
    ...
    'active': false,
    'dateModification': Timestamp.fromDate(
      DateTime.now(),
    ),
  });
}
