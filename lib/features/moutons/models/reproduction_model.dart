...
class ReproductionModel {
  ...
  final ReproductionStatut statut;
  ...
  const ReproductionModel({
    ...
    this.observations = "",
    this.statut = ReproductionStatut.saillie,
    required this.dateCreation,
    this.dateModification,
  });

  /// Nombre de jours écoulés depuis la saillie
  int get joursGestation {
    return DateTime.now().difference(dateSaillie).inDays;
  }

  /// Durée moyenne de gestation d'une brebis
  int get dureeGestation => 150;

  /// Nombre de jours restants avant la mise bas
  int get joursRestants {
    final reste = dureeGestation - joursGestation;
    return reste < 0 ? 0 : reste;
  }

  /// Progression de la gestation (0.0 → 1.0)
  double get progression {
    final valeur = joursGestation / dureeGestation;
  ...
  bool get miseBasPrevueAujourdHui =>
      joursRestants == 0 &&
          statut == ReproductionStatut.gestation;

  /// Gestation en retard
  bool get enRetard =>
      joursGestation > dureeGestation &&
          statut == ReproductionStatut.gestation;
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      "datePrevueMiseBas": datePrevueMiseBas.toIso8601String(),
  ...
  factory ReproductionModel.fromMap(
    ...
    return ReproductionModel(
      ...
      dateSaillie: DateTime.parse(map["dateSaillie"]),
      belierId: map["belierId"] ?? "",
      nomBelier: map["nomBelier"] ?? "",
      raceBelier: map["raceBelier"] ?? "",
      proprietaireBelier: map["proprietaireBelier"] ?? "",
      datePrevueMiseBas: DateTime.parse(
        map["datePrevueMiseBas"],
      ),
      dateMiseBas: map["dateMiseBas"] != null
          ? DateTime.parse(map["dateMiseBas"])
          : null,
      nombreAgneaux: map["nombreAgneaux"] ?? 0,
      nombreMales: map["nombreMales"] ?? 0,
      nombreFemelles: map["nombreFemelles"] ?? 0,
      observations: map["observations"] ?? "",
      statut: ReproductionStatut.values.firstWhere(
            (e) => e.name == map["statut"],
        orElse: () => ReproductionStatut.saillie,
      ),
  ...
  ReproductionModel copyWith({
    ...
    return ReproductionModel(
      ...
      nomBelier: nomBelier ?? this.nomBelier,
      raceBelier: raceBelier ?? this.raceBelier,
      proprietaireBelier:
      proprietaireBelier ?? this.proprietaireBelier,
      datePrevueMiseBas:
      datePrevueMiseBas ?? this.datePrevueMiseBas,
