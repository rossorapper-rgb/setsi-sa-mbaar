class MoutonModel {
  ...
  /// Code métier du mouton
  String get code => numeroIdentification;
  ...
  int get ageEnMois {
    ...
    int mois =
        (maintenant.year - dateNaissance!.year) * 12 +
            (maintenant.month - dateNaissance!.month);

    if (maintenant.day < dateNaissance!.day) {
      mois--;
    }
  ...
  factory MoutonModel.fromMap(Map<String, dynamic> map) {
    return MoutonModel(
      id: map['id'] ?? '',
      bergerieId: map['bergerieId'] ?? '',
      nom: map['nom'] ?? '',
      numeroIdentification: map['numeroIdentification'] ?? '',
      race: map['race'] ?? '',
      sexe: map['sexe'] ?? '',
