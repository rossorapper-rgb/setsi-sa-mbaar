...
_firestore.collection('reproductions');

CollectionReference<Map<String, dynamic>>
get _moutonCollection =>
_firestore.collection('moutons');

CollectionReference<Map<String, dynamic>>
get _miseBasCollection =>
_firestore.collection('mises_bas');

//====================================================
// CREER UNE REPRODUCTION
//====================================================

Future<ReproductionModel> creerReproduction({
...
required String proprietaireBelier,
String observations = "",
}) async {
...
String observations = "",
  ...
  final List<MoutonModel> agneaux = [];
  ...
  for (int i = 0; i < total; i++) {
    ...
    final sexe = i < nombreMales
        ? "Mâle"
        : "Femelle";

    agneaux.add(
      MoutonModel(
        id: _moutonCollection.doc().id,
        bergerieId: mere.bergerieId,
        nom: "Agneau ${i + 1}",
        numeroIdentification: code,
        race: mere.race,
        sexe: sexe,
        dateNaissance: dateMiseBas,
        poids: poidsNaissance,
        couleur: couleur,
        photoUrl: "",
        actif: true,
        dateCreation: DateTime.now(),
  ...
  Future<void> enregistrerMiseBas({
    required String reproductionId,
    required DateTime dateMiseBas,
  ...
  Future<List<MiseBasModel>> getMisesBasDuMouton(
      String moutonId,
      ) async {
    final snapshot =
  ...
  Future<MiseBasModel?> getMiseBas(
      String id,
      ) async {
    final doc =
    await _miseBasCollection
  ...
  getToutesLesReproductions() async {
    final snapshot =
    await _reproductionCollection.get();

    final liste = snapshot.docs
        .map(
          (doc) => ReproductionModel.fromMap(
        doc.data(),
      ),
    )
        .toList();
